...
public class SockIOPool {
	...
	private int hashingAlg = KETAMA_HASH; // default to using the native hash as
											// it is the fastest
	...
	/**
	 * NATIVE_HASH = 0 OLD_COMPAT_HASH = 1 NEW_COMPAT_HASH = 2 KETAMA_HASH = 3
	 * 
	 * @param key
	 * @return
	 */
	...
	public static Long findPointFor(Long hashK) {
		// ceilingKey returns the least key greater than or equal to the given
		// key,
	...
	/**
	 * Sets the list of all cache servers and weights for each string in the
	 * array that should be in the form ip:port\tweight
	 * 
	 * @param servers
	 */
	...
	public synchronized void initialize() {
		...
		maxCreate = (poolMultiplier > minConn) ? minConn : minConn
				/ poolMultiplier; // only create up to maxCreate connections at
									// once
		...
		for (int i = 0; i < servers.length; i++) {
			...
			for (int j = 0; j < initConn; j++) {
				...
				log.debug("++++ created and added socket: " + socket.toString()
						+ " for host " + servers[i]);
			}
		}
	...
	protected SockIO createSocket(String host) {
		...
		if (failover && hostDead.containsKey(host)
		...
		if (socket == null) {
			...
			log.warn("++++ ignoring dead host: " + host + " for " + expire
					+ " ms");
			...
			if (aSockets != null && !aSockets.isEmpty()) {
				...
				for (Iterator i = aSockets.keySet().iterator(); i.hasNext();) {
					...
					if (socket.isConnected()) {
						...
						log.error("++++ socket in avail pool is not connected: "
	 ...
	 */
	@SuppressWarnings("unchecked")
	protected synchronized void addSocketToPool(Map pool, String host,
			SockIO socket) {
	 ...
	 *            socket to remove
	 */
	protected synchronized void removeSocketFromPool(Map pool, String host,
			SockIO socket) {
		if (pool.containsKey(host)) {
			Map sockets = (Map) pool.get(host);
			if (sockets != null) {
	...
	public static class SockIO {
		...
		public SockIO(SockIOPool pool, String host, int port, int timeout,
			...
			sock = (connectTimeout > 0) ? getSocket(host, port, connectTimeout)
		...
		public SockIO(SockIOPool pool, String host, int timeout,
			...
			sock = (connectTimeout > 0) ? getSocket(ip[0],
					Integer.parseInt(ip[1]), connectTimeout) : new Socket(
					ip[0], Integer.parseInt(ip[1]));
		...
		protected static Socket getSocket(String host, int port, int timeout)
			...
			// Create a new thread which will attempt to connect to host:port,
			...
			throw new IOException("Could not connect for " + timeout
					+ " milliseconds");
		}
...
//			}
	...
	/**
	 * Thread to attempt connection. This will be polled by the main thread. We
	 * run the risk of filling up w/ threads attempting connections if network
	 * is down. However, the falling off mech in the main code should limit
	 * this.
	 * 
	 * @author greg whalin <greg@meetup.com>
	 * @version 1.2
	...
	static class ConnectThread extends Thread {
		...
		// logger
		private static Logger log = Logger.getLogger(ConnectThread.class
				.getName());
